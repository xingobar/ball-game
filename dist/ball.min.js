var canvas = document.getElementById('ball_canvas'),
	...
	probability = 0,
	isToolShow = !1,
	toolIntervalSeconds = 5e3,
	toolTime = 5,
	toolAddPaddleWidth = 50,
	toolInitY = -20,
	toolSetInterval = null,
	paddleHeight = 10,
	paddleWidth = 75,
	paddleOriginWidth = 75,
...
function toolInterval() {
	toolSetInterval = setInterval(function() {
		...
		0 < toolTime ||
			...
			(toolTime = 5),
			paddleWidth > paddleOriginWidth && ((paddleWidth -= brickAddedWidth), (brickAddedWidth = 0)));
	}, toolIntervalSeconds);
}
function toolCollision() {
	toolY + toolDy > canvas.height && ((toolY = toolInitY), (isToolShow = !1));
	toolX > paddleX &&
		toolX < paddleWidth + paddleX &&
		toolY + toolRadius >= canvas.height &&
		((toolY = toolInitY),
		(paddleWidth += toolAddPaddleWidth),
		(isToolShow = !1),
		(brickAddedWidth += toolAddPaddleWidth),
		toolSetInterval && clearInterval(toolSetInterval),
		toolInterval());
	isToolShow || (toolX = canvas.width - getRandomInt(350));
}
function drawToolRemainTime(a) {
	ctx.font = '16px Arial';
	ctx.fillStyle = '#0095DD';
	ctx.fillText('Tool : ' + a, canvas.width - 120, 20);
}
function drawLives() {
	ctx.font = '16px Arial';
	ctx.fillStyle = '#0095DD';
...
function drawTool() {
	ctx.beginPath();
	ctx.arc(toolX, toolY, toolRadius, 0, 2 * Math.PI, !1);
	ctx.fillStyle = 'blue';
	ctx.fill();
	ctx.fillStyle = 'white';
...
function draw() {
	...
	drawScore();
	drawLives();
	6 <= probability && !isToolShow && (isToolShow = !0);
	isToolShow && (drawTool(), toolCollision());
	bricksCollision();
	toolSetInterval && 0 < toolTime && drawToolRemainTime(toolTime);
	y + dy < ballRadius
		? (dy = -dy)
		: y + dy > canvas.height - ballRadius &&
			(x > paddleX && x < paddleX + paddleWidth
				? (dy = -dy)
				: (lives--,
					(paddleWidth = paddleOriginWidth),
					(brickAddedWidth = 0),
					toolInterval && (clearInterval(toolInterval), (toolTime = 5)),
					0 >= lives
